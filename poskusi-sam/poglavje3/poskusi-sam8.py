...
def train(model, xs, ys, learning_rate=0.001, n_epochs=1000):
    for k in range(n_epochs):
        ...
        print(f"epoch: {k}, w: {model.w}, b: {model.b}")
        # if k % 50 == 0:
...
stopnja_ucenja = [0.001, 0.1]

for i in range(len(stopnja_ucenja)):
    print(f"stopnja ucenja: {stopnja_ucenja[i]}")
    m = train(LinReg(), xs, ys, learning_rate=stopnja_ucenja[i], n_epochs=100)
    print("----------")
